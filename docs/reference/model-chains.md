...
An absent or empty `model_chains` field preserves the pre-chain schema-4 identity. Older workers can read those direct-only snapshots. A populated chain changes the identity and must fail closed on a worker that cannot interpret it. Changing the schema stamp merely to activate tolerant parsing would let an older worker discard the chain policy, including an explicitly unavailable model, and serve a different route.
