...
from exp.runtime.gateway.snapshot_file_windows import windows_snapshot_observation
...
def prepare_snapshot_file(
    ...
    """Read once and retain secure handles through a caller's short transaction fence."""
    with _snapshot_observation(root, relative_path) as (stream, identities):
        ...
        try:
            prepared._content = (
                None if stream is None else _read_stream_bytes(stream, maximum_bytes)
            )
