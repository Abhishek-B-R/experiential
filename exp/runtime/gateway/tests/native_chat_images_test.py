...
@pytest.mark.parametrize("stream", [False, True])
def test_generated_image_is_delivered_by_served_chat(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, stream: bool
) -> None:
    """An image-only paid completion preserves its bytes and usage on both chat shapes."""
    received: list[JsonObject] = []
    image = {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{_PNG_BASE64}"}}

    class Provider(BaseHTTPRequestHandler):
        """Serve one synthetic image without any external provider spending."""

        def do_POST(self) -> None:  # noqa: N802
            """Return a complete raster image and provider token usage."""
            received.append(json.loads(self.rfile.read(int(self.headers["content-length"]))))
            frames = (
                _provider_frame(
                    {
                        "choices": [
                            {"index": 0, "delta": {"images": [image]}, "finish_reason": "stop"}
                        ]
                    }
                )
                + _provider_frame(
                    {"choices": [], "usage": {"prompt_tokens": 11, "completion_tokens": 100}}
                )
                + b"data: [DONE]\n\n"
            )
            self.send_response(200)
    ...
    try:
        ...
        if stream:
            ...
            assert any(
                frame.get("choices") and frame["choices"][0]["delta"].get("images") == [image]
                for frame in frames
            )
            assert any(
                frame.get("usage", {}).get("completion_tokens") == 100
                for frame in frames
                if frame.get("usage")
            )
            assert response.text.endswith("data: [DONE]\n\n")
