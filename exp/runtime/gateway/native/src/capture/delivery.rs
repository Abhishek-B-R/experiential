...
impl Delivery {
    ...
    pub(super) fn submit_wait(&self, value: Record, wire: Option<WireResponse>) -> bool {
        ...
        self.enqueue(value, wire, Some(completed), None) && outcome.recv().unwrap_or(false)
    }

    /// Transfer the collector's existing source charge only after delivery reserves capacity.
    pub(super) fn submit_wait_handoff(
        &self,
        value: Record,
        wire: Option<WireResponse>,
        handoff: super::collector::Handoff,
    ) -> bool {
        let (completed, outcome) = mpsc::sync_channel(1);
        self.enqueue(value, wire, Some(completed), Some(handoff)) && outcome.recv().unwrap_or(false)
    }
    ...
    fn enqueue(
        ...
        wire: Option<WireResponse>,
        completed: Option<mpsc::SyncSender<bool>>,
        handoff: Option<super::collector::Handoff>,
    ) -> bool {
        ...
        drop(capacity);
        // Source and destination briefly overlap their charges; there is never
        // an uncharged producer holding accepted content while capacity waits.
        drop(handoff);
        let item = Pending {
            value,
            wire,
