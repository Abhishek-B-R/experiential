...
from exp.simulation.ingest.vendor_trace import SOURCE_SPAN_ATTRIBUTE
...
def apply_gateway_metrics(result: TraceNormalizationResult) -> TraceNormalizationResult:
    ...
    return replace(result, traces=tuple(_measured_trace(trace) for trace in result.traces))


def _measured_trace(trace: Trace) -> Trace:
