...
from exp.common.traces.ingest.persistence import ingest_traces
...
_TRACE_SOURCES = tuple(sorted((*CANONICAL_TRACE_SOURCES, "gateway")))
...
def ingest(
    ...
    try:
        ...
        if source not in _TRACE_SOURCES:
            raise ValueError(f"unsupported source; choose one of: {', '.join(_TRACE_SOURCES)}")
        if source == "gateway":
            if identity is None:
    ...
    _console.print(f"{result.trace_count} accepted traces · {len(result.issues)} excluded records")
